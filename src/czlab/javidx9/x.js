...
let [camcnr,camseg]=[1,1];

function _draw(){
  cls()
  //direction
  let camang=camz*ROAD[camcnr].tu;
  let [xd,yd,zd]=[-camang,0,1];
  //skew camera to account for direction
  let [cx,cy,cz]=skew(camx,camy,camz,xd,yd);
  //cursor, relative to skewed camera
  let [x,y,z]= [-cx,-cy+1,-cz+1];
  //road position
  let [cnr,seg]=[camcnr,camseg];
  //draw forward
  for(let i=1;i<30;++i){
  //for i=1,30 do
    //project
    let [px,py,scale]=project(x,y,z);
    //draw road
    let width=3*scale;
