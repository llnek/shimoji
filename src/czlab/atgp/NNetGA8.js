		...
		function gaNNet(popSize, mutationRate, crossOverRate, numWeights, splitPoints){
			return {
				...
				tournamentSelection(N){
					...
					for(let t,i=0;i<N;++i){
						t = _.randInt2(0, this.popSize - 1);
						if(this.vecPop[t].fitness > bestFitnessSoFar){
							chosenOne = t;
							bestFitnessSoFar = this.vecPop[t].fitness;
						}
					}
				...
				cycle(old){
					...
					while(vecNewPop.length < this.popSize){
						let mum = this.tournamentSelection(TOURNAMENT_COMPETITORS);
						let dad = this.tournamentSelection(TOURNAMENT_COMPETITORS);
